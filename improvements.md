...
- Four large leader artwork previews appear under the guide paragraph in one row: `11.jpg`, `12.png`, `13.png`, and `14.png`. The images use full-card display so the top and bottom of the artwork are not cropped.
...
|---|---|---|
| #1 | Survivor | 20 referrals |
| #2 | jocker | 16 referrals |
| #3 | tommyg | 12 referrals |
| #4 | Raven | 9 referrals |
| #5 | Nova | 7 referrals |
| #6 | Orion | 5 referrals |
| #7 | Valkyr | 4 referrals |
| #8 | Atlas | 3 referrals |
| #9 | Echo | 2 referrals |
| #10 | Lyra | 1 referral |
...
Leaderboard rows are kept at a consistent height with a slightly smaller centered pack image. Only ranks #1 through #4 show the pack image. Ranks #5 through #10 keep the same center spacing but do not show an image. The hover pop-up preview was removed from the leaderboard pack images. The leaderboard list scrolls internally so the first six rows are visible first and the remaining rows can be reached by scrolling. Invite milestone rows use a consistent minimum height so every line looks even.
